...
void main()
{
	...
	int i;
	firstarray(arr,s1,i);
	secondarray(brr,s2,i);
	margebotharray(arr,brr,crr,s1,s2,s3,i);
}
void firstarray(int*arr,int s1,int i)
{
...
void secondarray(int*brr,int s2,int i)
{
printf("enter element in array brr :- \n ");
	...
	void margebotharray(int*arr,int*brr,int*crr,int s1, int s2,int s3,int i)
	{
		for(i=0; i<s1; i++)
	{
		crr[i]=arr[i];
